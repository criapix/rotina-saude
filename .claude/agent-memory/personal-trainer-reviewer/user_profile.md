---
name: Perfil do usuário principal
description: Informações sobre o aluno cujas séries são revisadas — restrições físicas, objetivos e características do programa
type: user
---

Aluno masculino, pratica ciclismo (Z2 em dias de treino, Ter/Qua/Sex) e musculação 4x/semana (Seg/Ter/Qua/Qui). **Bioimpedância 04/04/2026:** 78,8 kg | BF% 11,7% (próximo ao limite inferior 11%) | FFM 65,1 kg | SMM 39,3 kg (novos recordes absolutos).

**Restrições físicas documentadas:**
- Rotura parcial do supraespinhal (manguito rotador) — ombro direito presumido
- Osteíte reacional ativa na tuberosidade maior
- Tendinose do subescapular documentada
- Coluna cervicotorácica degenerativa (C5-C7)
- Risco de síndrome do duplo esmagamento ulnar (double crush) no nervo ulnar — usa luvas com padding ulnar no pedal

**Objetivos declarados:**
1. Ganho de peitoral e braço
2. Redução de gordura abdominal
3. Fortalecimento de core
4. Melhora de força para ciclismo

**Nível estimado:** Intermediário (terminologia técnica compreendida, programa complexo com periodização)

**Divisão de treino:** A/B/C/D (Upper A / Lower A / Upper B / Lower B), Seg-Qui

**Equipamentos disponíveis:** Academia com: chest press máquina, remada cavalinho máquina, remada sentada cabo, puxada lat, tríceps pulley, leg press 45°, mesa flexora, cadeira abdutora, cadeira extensora, panturrilha em pé, hip thrust máquina, crunch máquina, máquina de rotação de tronco, cabo com corda e barra reta. Tem elásticos para protocolo de ativação.
